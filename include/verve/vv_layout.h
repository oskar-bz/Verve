...
static inline vv_Size vv_fixed(float n) {
  return (vv_Size){VV_SIZE_FIXED, n, 0, 0};
}
static inline vv_Size vv_fit(void) { return (vv_Size){VV_SIZE_FIT, 0, 0, 0}; }
static inline vv_Size vv_grow(float w) {
  return (vv_Size){VV_SIZE_GROW, w, 0, 0};
}
static inline vv_Size vv_percent(float p) {
  return (vv_Size){VV_SIZE_PERCENT, p, 0, 0};
}
