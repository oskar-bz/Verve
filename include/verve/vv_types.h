...
static inline vv_Rect vv_rect(float x, float y, float w, float h) {
  return (vv_Rect){x, y, w, h};
}
static inline vv_Color vv_rgba(float r, float g, float b, float a) {
  return (vv_Color){r, g, b, a};
}
static inline vv_Color vv_rgb(float r, float g, float b) {
  return (vv_Color){r, g, b, 1.0f};
}
static inline vv_Edges vv_all(float n) { return (vv_Edges){n, n, n, n}; }
static inline vv_Edges vv_hv(float h, float v) {
  return (vv_Edges){h, v, h, v};
}
static inline vv_Corners vv_r(float n) { return (vv_Corners){n, n, n, n}; }
static inline vv_Corners vv_corners(float t, float r, float b, float l) {
  return (vv_Corners){t, r, b, l};
};
static inline vv_Mat23 vv_mat_identity(void) {
  return (vv_Mat23){1, 0, 0, 1, 0, 0};
}
static inline vv_Mat23 vv_scale(float s) {
  return (vv_Mat23){s, 0, 0, s, 0, 0};
}
...
static inline float vv_lerpf(float a, float b, float t) {
  return a + (b - a) * t;
}

// Map x from [in_lo, in_hi] onto [out_lo, out_hi] (no clamping). Guards a
// zero-width input range by returning out_lo — the natural "no information"
// fallback.
static inline float vv_remapf(float x, float in_lo, float in_hi, float out_lo,
                              float out_hi) {
  float d = in_hi - in_lo;
  return d == 0.0f ? out_lo : out_lo + (out_hi - out_lo) * ((x - in_lo) / d);
}

static inline vv_Vec2 vv_v2add(vv_Vec2 a, vv_Vec2 b) {
  return (vv_Vec2){a.x + b.x, a.y + b.y};
}
static inline vv_Vec2 vv_v2sub(vv_Vec2 a, vv_Vec2 b) {
  return (vv_Vec2){a.x - b.x, a.y - b.y};
}
static inline vv_Vec2 vv_v2scale(vv_Vec2 a, float s) {
  return (vv_Vec2){a.x * s, a.y * s};
}
