...
// The entire "brain" is fuzzy_score(): a function from (query, string) to an int,
// with no idea a UI exists. update() runs it over the dataset and sorts; view()
// just draws the ranked results. Because each result row is keyed by its item id,
// re-ranking as you type makes rows FLIP-spring to their new positions, and rows
// entering/leaving the top list fade — animation you never wrote.
//
...
static int fuzzy_score(const char *q, const char *s) {
  if (!*q) return 1; // empty query: everything matches, keep original order
  int score = 0, last = -2, si = 0;
  const char *qc = q;
  for (; s[si] && *qc; si++) {
    if (tolower((unsigned char)s[si]) == tolower((unsigned char)*qc)) {
      score += 10;
      if (last == si - 1) score += 15;                        // consecutive run
      if (si == 0 || s[si - 1] == '/' || s[si - 1] == '_' ||
          s[si - 1] == '.' || s[si - 1] == ' ')
  ...
  if (*qc) return -1000000;               // not all query chars consumed
...
static void rerank(Finder *f) {
  ...
  for (int i = 0; i < NITEMS; i++) {
    ...
    if (sc > -1000000) { f->ranked[f->nranked] = i; f->score[i] = sc; f->nranked++; }
  }
  g_sort = f;
...
static void update(void *st, vv_Event ev) {
  Finder *f = st;
  switch (ev.msg) {
  case MSG_QUERY: rerank(f); f->picked = -1; break;
  case MSG_MOVE: {
    int n = f->nranked < NSHOW ? f->nranked : NSHOW;
    f->sel += (int)ev.data.as_int;
    if (f->sel < 0) f->sel = 0;
    if (f->sel >= n) f->sel = n ? n - 1 : 0;
    break;
  }
  case MSG_PICK:
    if (f->sel < f->nranked) f->picked = f->ranked[f->sel];
    break;
  default: break;
  }
}
...
static void view(vv_Ctx *c, void *st) {
  ...
  VV_BOX(c, VV_LAYOUT(.dir = VV_COLUMN, .w = vv_grow(1), .h = vv_grow(1),
    ...
    VV_BOX(c, VV_LAYOUT(.dir = VV_ROW, .w = vv_grow(1), .cross = VV_ALIGN_CENTER,
      ...
      vv_text(c, "\xe2\x9d\xaf", VV_STYLE(.fg = t->accent_hi, .font_size = 18));
      VV_BOX(c, VV_LAYOUT(.w = vv_grow(1)), VV_STYLE(.bg = {0})) {
        vv_text_field(c, "q", f->query, sizeof f->query, "fuzzy find\xe2\x80\xa6", MSG_QUERY);
      }
    ...
    VV_BOX(c, VV_LAYOUT(.dir = VV_COLUMN, .w = vv_grow(1), .h = vv_grow(1), .gap = 3),
           VV_STYLE(.bg = {0})) {
      for (int i = 0; i < n; i++) {
        int id = f->ranked[i];
        bool sel = i == f->sel;
        bool pick = id == f->picked;
        char k[12]; snprintf(k, sizeof k, "i%d", id);
        VV_BOX(c, VV_LAYOUT(.dir = VV_ROW, .w = vv_grow(1), .cross = VV_ALIGN_CENTER,
                            .gap = 10, .padding = vv_hv(12, 8)),
               VV_STYLE(.bg = pick ? t->accent : (sel ? t->surface_hi : (vv_Color){0}),
                        .radius = vv_r(8))) {
          (void)k;
          vv_box_keyed(c, "dot", 3, VV_LAYOUT(.w = vv_fixed(6), .h = vv_fixed(6)),
                       VV_STYLE(.bg = sel ? t->accent_hi : t->text_muted, .radius = vv_r(3)));
          vv_end_box(c);
          vv_text(c, f->items[id],
...
static void seed(Finder *f) {
  static const char *dirs[] = {"src", "include/verve", ".config/nvim", "docs",
      "build", "examples", "tests", "backends", "vendor/stb", "assets/img"};
  static const char *stems[] = {"main", "config", "vv_layout", "vv_style", "index",
      "render", "parser", "widget", "theme", "utils", "server", "client", "cache",
      "readme", "notes", "todo", "spring", "context", "buffer", "shader"};
  static const char *exts[] = {"c", "h", "md", "toml", "lua", "txt", "glsl", "json"};
  ...
  for (int i = 0; i < NITEMS; i++) {
    ...
    snprintf(f->items[i], sizeof f->items[i], "%s/%s%d.%s",
             ...
             (int)((rng >> 15) % 90), exts[(rng >> 22) % (sizeof exts / sizeof *exts)]);
  }
  f->picked = -1;
  rerank(f);
}
...
int main(void) {
  ...
  while (vv_app_pump(app, &in)) {
    ...
    if (first) { vv_request_focus_next(&ctx); first = false; }
    ...
    bool up = ks[SDL_SCANCODE_UP], dn = ks[SDL_SCANCODE_DOWN], ent = ks[SDL_SCANCODE_RETURN];
    if (up && !up_prev)   { vv_emit(&ctx, MSG_MOVE, vv_pi(-1)); vv_invalidate(&ctx); }
    if (dn && !dn_prev)   { vv_emit(&ctx, MSG_MOVE, vv_pi(1));  vv_invalidate(&ctx); }
    if (ent && !ent_prev) { vv_emit(&ctx, MSG_PICK, VV_NO_PAYLOAD); vv_invalidate(&ctx); }
    up_prev = up; dn_prev = dn; ent_prev = ent;
