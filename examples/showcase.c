...
enum {
  ...
  MSG_POP_OPEN, MSG_POP_CLOSE, MSG_WRAP, MSG_SNIPPET,
};
...
static void update(void *st, vv_Event ev) {
  ...
  case MSG_POP_OPEN:  a->pop_open = true; break;
...
static uint32_t toolbar_button(vv_Ctx *c, const char *key, const char *glyph,
  ...
  vv_end_box(c);
  if (vv_clicked(c, id)) vv_emit(c, msg, VV_NO_PAYLOAD);
  return id;
}
...
static void view(vv_Ctx *c, void *st) {
  App *a = st;
  const vv_Theme *t = vv_theme();
  uint32_t m_file = 0, m_view = 0, m_win = 0; // menu-title handles, used by overlays
  ...
  VV_BOX(c, VV_LAYOUT(.dir = VV_COLUMN, .w = vv_grow(1), .h = vv_grow(1)),
    ...
    m_view = vv_menu_title(c, "view", "View");
    ...
    VV_BOX(c, VV_LAYOUT(.dir = VV_ROW, .w = vv_grow(1), .cross = VV_ALIGN_CENTER,
      ...
      a->tb_aa   = toolbar_button(c, "taa", "Aa", MSG_POP_OPEN);
  ...
  // ===== overlay layer: everything below paints on top ======================
  ...
  // View menu
  if (vv_menu_is_open(c, m_view)) {
    vv_Rect r = vv_node(c, m_view)->actual_rect;
  ...
  // Popover anchored under the "Aa" button.
  if (a->pop_open) {
    vv_Rect r = vv_node(c, a->tb_aa)->actual_rect;
    vv_popover_begin(c, "aapop", vv_v2(r.x, r.y + r.h + 4), 240, MSG_POP_CLOSE);
  ...
  // Tooltips (self-time on hover; paint on top here).
