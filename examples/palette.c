...
// a color in OKLCH (lightness / chroma / hue) — the space where equal steps look
// equal — and Verve generates a tint/shade ramp that's evenly spaced to the eye,
// not to the sRGB cube. Live WCAG contrast tells you if text will be readable.
//
...
typedef struct { float L, C, H; } State;
...
static void update(void *state, vv_Event ev) {
  State *s = state;
  switch (ev.msg) {
  case MSG_L: s->L = (float)ev.data.as_float; break;
  case MSG_C: s->C = (float)ev.data.as_float; break;
  case MSG_H: s->H = (float)ev.data.as_float; break;
  case MSG_PICK: s->L = (float)ev.data.as_float; break; // adopt a ramp step's L
  default: break;
  }
}
...
// A WCAG contrast badge: the ratio of `fg` on `bg`, tinted by whether it passes.
...
static void view(vv_Ctx *c, void *state) {
  ...
  VV_BOX(c, VV_LAYOUT(.dir = VV_COLUMN, .w = vv_grow(1), .h = vv_grow(1),
    ...
    vv_text(c, "OKLab Palette Studio", VV_STYLE(.fg = t->text, .font_size = 24));
    ...
    VV_BOX(c, VV_LAYOUT(.dir = VV_ROW, .w = vv_grow(1), .gap = 20),
      ...
      VV_BOX(c, VV_LAYOUT(.dir = VV_COLUMN, .w = vv_grow(1), .gap = 12),
             VV_STYLE(.bg = {0})) {
        VV_BOX(c, VV_LAYOUT(.w = vv_grow(1), .h = vv_fixed(200),
          ...
          vv_Color ink = contrast(base, vv_rgb(1, 1, 1)) >= contrast(base, vv_rgb(0, 0, 0))
                             ? vv_rgb(1, 1, 1) : vv_rgb(0, 0, 0);
      ...
      // Controls.
      VV_BOX(c, VV_LAYOUT(.dir = VV_COLUMN, .w = vv_fixed(340), .gap = 18,
                          .padding = vv_all(18)),
    ...
    VV_BOX(c, VV_LAYOUT(.dir = VV_ROW, .w = vv_grow(1), .h = vv_fixed(72),
                        .gap = 8),
           VV_STYLE(.bg = {0})) {
      for (int i = 0; i < NRAMP; i++) {
        ...
        vv_Style hov = {.transform = vv_scale(1.05f), .set = VV_STYLE_TRANSFORM};
        uint32_t id = vv_box_keyed(
            c, key, strlen(key),
            VV_LAYOUT(.w = vv_grow(1), .h = vv_grow(1)),
            VV_STYLE(.bg = sw, .radius = vv_r(10), .hover = &hov));
        vv_end_box(c);
        if (vv_clicked(c, id)) vv_emit(c, MSG_PICK, vv_pf((double)L));
      }
    }
  }
}
