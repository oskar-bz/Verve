...
static bool button_en(vv_Ctx *c, const char *key, const char *label,
                      bool enabled) {
...
static bool field_w(vv_Ctx *c, const char *key, char *buf, int cap,
                    const char *ph, float w) {
  bool ch;
...
static void task_counter(vv_Ctx *c) {
  title(c, "Counter");
  VV_BOX(c,
         ((vv_LayoutDecl){.dir = VV_ROW, .gap = 12, .cross = VV_ALIGN_CENTER}),
         (vv_Style){0}) {
    vv_box_keyed(c, "disp", 4,
                 (vv_LayoutDecl){.w = vv_fixed(80),
                                 .h = vv_fixed(36),
                                 .main = VV_ALIGN_CENTER,
                                 .cross = VV_ALIGN_CENTER},
                 (vv_Style){.bg = TH->surface, .radius = vv_r(6)});
    vv_text(c, vv_fmt(c, "%d", g_count),
            (vv_Style){.fg = TH->text, .font_size = 16});
    vv_end_box(c);
    if (vv_button(c, "inc", "Count"))
      g_count++;
  }
}
...
static void task_temp(vv_Ctx *c) {
  ...
  VV_BOX(c,
    ...
    if (field_w(c, "cel", g_cel, sizeof g_cel, "0", 90)) {
      char *end;
      double v = strtod(g_cel, &end);
      if (end != g_cel)
        snprintf(g_fah, sizeof g_fah, "%.1f", v * 9.0 / 5.0 + 32.0);
    }
    vv_label(c, "Celsius  =");
    if (field_w(c, "fah", g_fah, sizeof g_fah, "32", 90)) {
      char *end;
      double v = strtod(g_fah, &end);
      if (end != g_fah)
        snprintf(g_cel, sizeof g_cel, "%.1f", (v - 32.0) * 5.0 / 9.0);
    }
...
static void task_flight(vv_Ctx *c) {
  ...
  VV_BOX(c, ((vv_LayoutDecl){.dir = VV_COLUMN, .gap = 10, .w = vv_fixed(240)}),
    ...
    if (vv_button(c, "type", types[g_flight_type]))
      g_flight_type ^= 1;

    int t1, t2;
    bool v1 = parse_date(g_d1, &t1), v2 = parse_date(g_d2, &t2);
    field_w(c, "d1", g_d1, sizeof g_d1, "DD.MM.YYYY", 240);
    // Return date disabled for one-way.
    VV_BOX(c,
           ((vv_LayoutDecl){.w = vv_grow(1), .disabled = g_flight_type == 0}),
    ...
    if (button_en(c, "book", "Book", ok))
      g_booked = true;
    if (g_booked) {
      vv_Str msg = g_flight_type == 0
                       ? vv_fmt(c, "Booked one-way for %s.", g_d1)
                       : vv_fmt(c, "Booked %s → %s.", g_d1, g_d2);
...
static void task_timer(vv_Ctx *c, float dt) {
  if (g_elapsed < g_duration)
    g_elapsed = fminf(g_elapsed + dt, g_duration);
  title(c, "Timer");
  VV_BOX(c, ((vv_LayoutDecl){.dir = VV_COLUMN, .gap = 14, .w = vv_fixed(320)}),
    ...
    vv_label(c, vv_fmt(c, "%.1f s", (double)g_elapsed));
    VV_BOX(
        c,
        ((vv_LayoutDecl){.dir = VV_ROW, .gap = 10, .cross = VV_ALIGN_CENTER}),
        (vv_Style){0}) {
...
static void task_crud(vv_Ctx *c) {
  if (!g_crud_init) {
    strcpy(g_db[0].name, "Hans");
    strcpy(g_db[0].surname, "Emil");
    strcpy(g_db[1].name, "Max");
    strcpy(g_db[1].surname, "Mustermann");
    strcpy(g_db[2].name, "Roman");
    strcpy(g_db[2].surname, "Tisch");
    g_db_n = 3;
    g_crud_init = true;
  }
  title(c, "CRUD");
  VV_BOX(c,
         ((vv_LayoutDecl){
             .dir = VV_COLUMN, .gap = 10, .w = vv_grow(1), .h = vv_grow(1)}),
         (vv_Style){0}) {
    VV_BOX(c,
           ((vv_LayoutDecl){.dir = VV_ROW, .gap = 8, .cross = VV_ALIGN_CENTER}),
           (vv_Style){0}) {
      vv_label(c, "Filter:");
    ...
    VV_BOX(c,
           ((vv_LayoutDecl){
               .dir = VV_ROW, .gap = 14, .w = vv_grow(1), .h = vv_grow(1)}),
           (vv_Style){0}) {
      // List.
      VV_BOX(c,
        ...
        for (int i = 0; i < g_db_n; i++) {
          if (g_filter[0] &&
              strncmp(g_db[i].surname, g_filter, strlen(g_filter)) != 0)
            continue;
          vv_Str lbl = vv_fmt(c, "%s, %s", g_db[i].surname, g_db[i].name);
          vv_Str key = vv_fmt(c, "it%d", i);
          if (vv_list_item(c, key, lbl, g_sel == i)) {
            g_sel = i;
            strcpy(g_nm, g_db[i].name);
            strcpy(g_sn, g_db[i].surname);
          }
        }
      }
      // Editor.
      VV_BOX(c,
             ((vv_LayoutDecl){.dir = VV_COLUMN, .gap = 8, .w = vv_fixed(220)}),
             (vv_Style){0}) {
        VV_BOX(c,
               ((vv_LayoutDecl){
                   .dir = VV_ROW, .gap = 8, .cross = VV_ALIGN_CENTER}),
               (vv_Style){0}) {
          vv_label(c, "Name:");
          field_w(c, "nm", g_nm, sizeof g_nm, "", 150);
        }
        VV_BOX(c,
               ((vv_LayoutDecl){
                   .dir = VV_ROW, .gap = 8, .cross = VV_ALIGN_CENTER}),
               (vv_Style){0}) {
          vv_label(c, "Surname:");
          field_w(c, "sn", g_sn, sizeof g_sn, "", 130);
        }
      }
    }
    VV_BOX(c, ((vv_LayoutDecl){.dir = VV_ROW, .gap = 8}), (vv_Style){0}) {
      if (button_en(c, "create", "Create", g_nm[0] || g_sn[0]) &&
          g_db_n < 128) {
        strcpy(g_db[g_db_n].name, g_nm);
        strcpy(g_db[g_db_n].surname, g_sn);
        g_db_n++;
      }
      if (button_en(c, "update", "Update", g_sel >= 0)) {
        strcpy(g_db[g_sel].name, g_nm);
        strcpy(g_db[g_sel].surname, g_sn);
      }
      if (button_en(c, "delete", "Delete", g_sel >= 0)) {
        for (int i = g_sel; i < g_db_n - 1; i++)
          g_db[i] = g_db[i + 1];
        g_db_n--;
        g_sel = -1;
        g_nm[0] = g_sn[0] = 0;
...
static void task_circle(vv_Ctx *c) {
  ...
  VV_BOX(c,
         ...
         (vv_Style){0}) {
    VV_BOX(c,
           ((vv_LayoutDecl){.dir = VV_ROW, .gap = 8, .main = VV_ALIGN_CENTER}),
           (vv_Style){0}) {
      if (button_en(c, "undo", "Undo", g_hist_i > 0)) {
        g_hist_i--;
        g_csel = -1;
      }
      if (button_en(c, "redo", "Redo", g_hist_i < g_hist_n - 1)) {
        g_hist_i++;
        g_csel = -1;
      }
    }
...
static void task_cells(vv_Ctx *c) {
  ...
  VV_BOX(c,
    ...
    // Header row.
    VV_BOX(c,
           ((vv_LayoutDecl){.dir = VV_ROW, .gap = 1, .padding = vv_hv(1, 1)}),
           (vv_Style){0}) {
      ...
      for (int col = 0; col < CELL_COLS; col++) {
        ...
        char k[8];
        snprintf(k, sizeof k, "h%d", col);
        vv_box_keyed(c, k, 0,
                     (vv_LayoutDecl){.w = vv_fixed(88),
                                     .h = vv_fixed(26),
                                     .main = VV_ALIGN_CENTER,
                                     .cross = VV_ALIGN_CENTER},
    ...
    for (int r = 0; r < CELL_ROWS; r++) {
      ...
      VV_BOX(c,
             ((vv_LayoutDecl){.dir = VV_ROW, .gap = 1, .padding = vv_hv(1, 0)}),
             (vv_Style){0}) {
        char rl[4];
        snprintf(rl, sizeof rl, "%d", r + 1);
        vv_box_keyed(c, rk, 0,
                     (vv_LayoutDecl){.w = vv_fixed(36),
                                     .h = vv_fixed(34),
                                     .main = VV_ALIGN_CENTER,
                                     .cross = VV_ALIGN_CENTER},
                     (vv_Style){.bg = TH->surface_hi});
        ...
        for (int col = 0; col < CELL_COLS; col++) {
          char ck[12];
          snprintf(ck, sizeof ck, "c%d_%d", r, col);
                         ...
                         .border_color = TH->border});
          {
            if (editing) {
              ...
              char fk[16];
              snprintf(fk, sizeof fk, "e%d_%d", r, col);
...
static void build(vv_Ctx *c, float dt) {
  VV_BOX(c, ((vv_LayoutDecl){.dir = VV_ROW, .w = vv_grow(1), .h = vv_grow(1)}),
    ...
    VV_BOX(c,
      ...
      vv_box_keyed(c, "sp", 1, (vv_LayoutDecl){.h = vv_fixed(8)},
                   (vv_Style){0});
      vv_end_box(c);
      for (int i = 0; i < 7; i++) {
        char k[8];
        snprintf(k, sizeof k, "t%d", i);
        if (vv_list_item(c, k, TASKS[i], g_task == i))
          g_task = i;
      }
    }
    // Content.
